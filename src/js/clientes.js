...
function clienteWhatsApp(c) {
  ...
  const parts = [
    ...
    '',
    balance > 0
      ? 'Le recordamos que tiene un saldo pendiente de ' + fmt(balance) +
        ' con ' + CFG.biz + '.\nPor favor comuniquese con nosotros para coordinar el pago.'
      : 'Gracias por ser cliente de ' + CFG.biz + '.',
    '',
    CFG.phone ? 'Tel: ' + CFG.phone : '',
  ];
  const msg = parts.filter(function(l){ return l !== null && l !== undefined; }).join('\n');
