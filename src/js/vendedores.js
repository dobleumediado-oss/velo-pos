...
async function _venApplyPeriod() {
  ...
  _venRange = { from, to };
  await veloRepaint(() => renderVendedores(document.getElementById('page')));
}
...
async function vendedoresSaveSeller(id) {
  ...
  if(!r?.ok){toast(r?.error||'No se pudo guardar','err');return;}closeModal();toast('✓ Colaborador guardado');veloRepaint(() => renderVendedores(document.getElementById('page')));
}
async function vendedoresToggle(id,active){const r=await window.api.salespeople.toggle({id,active,requestUserId:user.id});if(!r?.ok){toast(r.error,'err');return;}veloRepaint(() => renderVendedores(document.getElementById('page')));}
...
async function vendedoresSaveExternal(){
  ...
  const r=await window.api.salespeople.createExternalSale({data,requestUserId:user.id});if(!r?.ok){toast(r.error,'err');return;}closeModal();toast('✓ Venta externa registrada y lista para comisión');_venTab='externas';veloRepaint(() => renderVendedores(document.getElementById('page')));
}
