...
async function renderServicio(el) {
  ...
  el.innerHTML = `
    <div class="sec-hdr">
      ...
      ${['admin','superadmin'].includes(user?.role)?`<button class="btn btn-out" id="svc-portal-config">${svg('link')} Portal clientes</button><button class="btn btn-out" id="svc-techs">${svg('users')} Técnicos</button>`:''}
...
function svcRenderDetail(order) {
  ...
  openModal(`<div style="display:flex;justify-content:space-between;gap:12px"><div><div class="modal-title">${svcEsc(order.number)}</div><div class="modal-sub">${svcEsc(order.customer_name)} · ${svcEsc(order.device_desc)} · ${svcEsc(order.imei||order.serial||'sin identificador')}</div></div>${svcBadge(order.workflow_status)}</div>
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin:10px 0"><div><button class="btn btn-out btn-sm" id="svc-print">${svg('printer')} Documento</button> <button class="btn btn-out btn-sm" id="svc-portal">${svg('link')} Portal / QR</button> <button class="btn btn-out btn-sm" id="svc-whatsapp">WhatsApp</button></div><div class="tb">Total: <span id="svc-total">${fmt(items.reduce((s,i)=>s+Number(i.qty)*Number(i.unit_price),0))}</span></div></div>
...
async function svcOpenPortalConfig(){
  const res=await window.api.serviceOrders.getPortalConfig({requestUserId:user?.id});
  if(!res?.ok)return toast(res?.error||'No se pudo leer la configuración','err');
  const cfg=res.data;
  const health=cfg.local_status||{};
  openModal(`<div class="modal-title">Portal de clientes por Tailscale Funnel</div><div class="modal-sub">La base sigue dentro de la PC servidor; solamente se publica el portal de seguimiento.</div><div class="alrt ${health.online?'b':'a'}" style="margin-top:14px"><div class="alrt-dot ${health.online?'b':'a'}"></div><div><div class="alrt-title">Portal local: ${health.online?'activo':'sin respuesta'}</div><div class="alrt-sub">${health.online?`Respondió en ${Number(health.response_ms)||0} ms`:`${svcEsc(health.detail||'No disponible')}. Verifica que VELO Server Service esté iniciado.`}</div></div></div><div class="alrt b"><div class="alrt-dot b"></div><div><div class="alrt-title">1. En la PC servidor</div><div class="alrt-sub">Ejecuta una vez: <code>${svcEsc(cfg.funnel_command)}</code></div></div></div><div class="fg"><label class="lbl">2. URL HTTPS entregada por Tailscale</label><input class="inp no-uppercase" id="svc-portal-base" data-uppercase="off" placeholder="https://velo-servidor.tu-tailnet.ts.net" value="${svcEsc(cfg.base_url||'')}"><div class="ts">No agregues /r ni el número de la orden.</div></div><div class="g2"><div class="fg"><label class="lbl">Vigencia de cada enlace (días)</label><input class="inp" id="svc-portal-days" type="number" min="1" max="3650" value="${Number(cfg.link_days)||365}"></div><label style="display:flex;align-items:center;gap:8px;margin-top:24px"><input type="checkbox" id="svc-portal-enabled" ${cfg.enabled?'checked':''}> Portal habilitado</label></div><div class="card" style="padding:12px"><div class="tb">Dirección local del portal</div><div class="ts">http://127.0.0.1:${Number(cfg.port)||8787}/health · Negocio: ${svcEsc(cfg.business_id)}</div></div><div class="modal-foot" style="justify-content:space-between"><button class="btn btn-out" id="svc-portal-refresh">Volver a comprobar</button><div><button class="btn btn-out" onclick="closeModal()">Cerrar</button> <button class="btn btn-dark" id="svc-portal-save">Guardar configuración</button></div></div>`, 'modal-lg');
  document.getElementById('svc-portal-refresh').onclick=svcOpenPortalConfig;
  document.getElementById('svc-portal-save').onclick=async()=>{const saved=await window.api.serviceOrders.savePortalConfig({requestUserId:user?.id,base_url:document.getElementById('svc-portal-base').value.trim(),link_days:Number(document.getElementById('svc-portal-days').value),enabled:document.getElementById('svc-portal-enabled').checked});if(!saved?.ok)return toast(saved?.error||'No se pudo guardar','err');toast('✓ Portal configurado','ok');closeModal();};
}
