...
function itemsTable(order) {
  ...
  return `<section class="card"><h2>Presupuesto / trabajo realizado</h2><table><thead><tr><th>Descripción</th><th>Cant.</th><th>Importe</th></tr></thead><tbody>${order.items.map(item => `<tr><td>${esc(item.description)}</td><td>${Number(item.qty)}</td><td>${money(Number(item.qty)*Number(item.unit_price))}</td></tr>`).join('')}</tbody></table><div class="total">${money(order.quote_amount)}</div></section>`;
}
