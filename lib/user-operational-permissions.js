...
const MANAGED_MODULE_KEYS = new Set([
  'dash', 'pos', 'credito', 'preventa', 'inventario', 'servicio', 'compras',
  'clientes', 'crm', 'ventas', 'devoluciones', 'vendedores', 'comisiones',
  'nomina', 'caja', 'gastos', 'bancos', 'contabilidad', 'sucursales',
  'vehiculos', 'mantenimiento', 'envios', 'conduce', 'reportes', 'impresion',
]);
...
function parseModulePermissions(user) {
...
function modulePermission(user, key, fallback = false) {
  if (isAdminUser(user)) return true;
  const permissions = parseModulePermissions(user);
  if (!Object.prototype.hasOwnProperty.call(permissions, key)) return fallback;
  return flagEnabled(permissions[key], false);
}

function canManageInventory(user) {
  return modulePermission(user, 'inventario', flagEnabled(user?.can_manage_inventory, false));
}

function canManageCustomers(user) {
  // Clientes siempre fue un módulo operativo base para caja. Una política
  // individual explícita puede concederlo o bloquearlo.
  return modulePermission(user, 'clientes', String(user?.role || '').toLowerCase() === 'cajero');
}
...
function canSellOnCredit(user) {
  ...
  return modulePermission(user, 'credito', flagEnabled(user?.can_sell_credit, true));
}
