...
function getPendingInvoices(db, customerId) {
  ...
  const documentSelect = [
    ...
    ? `s.${column}` : `${fallback} AS ${column}`).join(',\n           ');
  const effectiveSaleDate = saleColumns.has('sale_date')
    ? "COALESCE(NULLIF(s.sale_date,''),date(s.created_at))"
    : 'date(s.created_at)';

  const sales = db.prepare(`
    SELECT s.id, s.total, s.subtotal, s.tax_amt, s.discount_amt,
           s.created_at, ${effectiveSaleDate} AS sale_date, s.notes, s.ncf, s.status,
           ${documentSelect},
    ...
    ORDER BY ${effectiveSaleDate} ASC, s.id ASC
  `).all(customerId);
