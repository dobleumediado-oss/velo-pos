...
function buildOpenCashBlock(openCash, requestTerminalId = '', terminalNames = {}) {
  if (!openCash) return null;
  const openTerminalId = String(openCash.terminal_id || '').trim();
  const currentTerminalId = String(requestTerminalId || '').trim();
  const isCurrentTerminal = !!openTerminalId && !!currentTerminalId
    && openTerminalId === currentTerminalId;
  const terminalName = openTerminalId
    ? String(terminalNames?.[openTerminalId] || '').trim()
    : '';
  const cashier = String(openCash.cajero || '').trim();
  const openedAt = [openCash.open_date, openCash.open_time]
    .map(value => String(value || '').trim()).filter(Boolean).join(' ');
  const location = isCurrentTerminal
    ? 'en esta terminal'
    : openTerminalId
      ? `en otra terminal${terminalName ? ` (${terminalName})` : ''}`
      : 'en una sesión anterior sin terminal identificada';
  const instruction = isCurrentTerminal
    ? 'Actualiza la pantalla de Caja y cierra esa sesión antes de continuar.'
    : 'La barra superior muestra el estado de esta terminal; revisa la sección Caja y cierra la otra sesión antes de continuar.';
  return {
    code: 'OPEN_CASH_SESSION',
    error: `Hay una caja abierta ${location}${cashier ? `, a nombre de ${cashier}` : ''}${openedAt ? ` desde ${openedAt}` : ''}. ${instruction} ALL IN ONE no puede ejecutarse mientras exista una caja operativa abierta.`,
    openCash: {
      id: Number(openCash.id) || null,
      cashier,
      openedAt,
      terminalId: openTerminalId,
      terminalName,
      isCurrentTerminal,
    },
  };
...
// Orden de borrado del núcleo operativo reemplazado por ALL IN ONE. Las tablas
// fiscales forman parte del reset: conservar sus rangos/contadores después de
// reemplazar las ventas impediría indicar limpiamente el próximo NCF.
const EQUIPARTS_RESET_TABLES = Object.freeze([
  ...
  'accounting_entry_lines', 'accounting_entries', 'accounting_periods',
  'inventory_movements', 'product_price_history', 'ecf_log',
  'ncf_available_numbers', 'ncf_log', 'ncf_sequences', 'ncf_normalization_log', 'deliveries',
...
function wipeEquipartsResetTables(db) {
  const cleared = [];
  for (const table of EQUIPARTS_RESET_TABLES) {
    if (!tableExists(db, table)) continue;
    db.prepare(`DELETE FROM ${table}`).run();
    cleared.push(table);
  }
  // El catálogo y sus asignaciones se conservan, pero sus saldos materializados
  // deben volver a cero porque ya no existe ningún asiento que los sustente.
  if (tableExists(db, 'accounting_accounts')) {
    db.prepare("UPDATE accounting_accounts SET balance=0,updated_at=datetime('now') WHERE balance<>0").run();
  }
  // Los activos fijos sobreviven al ALL IN ONE. Si alguna depreciación histórica
  // apuntaba a un asiento eliminado, se conserva el registro auxiliar sin dejar
  // un identificador contable inexistente.
  if (tableExists(db, 'depreciation_entries')) {
    const columns = db.prepare('PRAGMA table_info(depreciation_entries)').all().map(column => column.name);
    if (columns.includes('accounting_entry_id')) {
      db.prepare('UPDATE depreciation_entries SET accounting_entry_id=NULL WHERE accounting_entry_id IS NOT NULL').run();
    }
  }
  return cleared;
}
