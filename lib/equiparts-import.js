...
function calculateInvoiceFiscalBreakdown(invoice, sourceItems = invoice?.items || []) {
  ...
  const detailTotal = round2(items.reduce((sum, item) =>
    sum + round2(item.line_total || (item.unit_price * item.qty)), 0));
  ...
  const factor = detailTotal > 0 ? targetGross / detailTotal : 1;
...
function validateEquipartsData(data) {
  ...
  for (const invoice of invoices.values()) {
    ...
    if (invoice.items.length) {
      ...
      if (invoice.total - detailTotal > 0.10) {
