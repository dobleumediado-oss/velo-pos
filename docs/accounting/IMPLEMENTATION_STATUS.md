...
| Motor de asientos automáticos (7.3) | 🟡 | **EN VIVO** venta/abono/gasto/anulación/devolución. **Falta compras**, notas de débito, ajustes de inventario, transferencias, comisiones, depreciación, retenciones |
...
| Cuentas por pagar (7.8) | 🟡 | Gastos "por pagar" operativo. **Contable falta** (ligado a compras/G1/G7) |
...
| Inventario contable (7.15) | 🟡 | Costo en venta (COGS/Inv) en el asiento. **Falta cuadre valor inventario operativo↔cuenta contable**, kardex valorizado contable |
| Nómina contable (7.16) | ⛔ | No existe nómina |
| Monedas (7.17) | ⛔ | Solo DOP; sin tasa/dif. cambiaria |
| Cierres (7.18) | ⛔ | Tabla `accounting_periods` existe; **sin lógica de cierre/bloqueo** |
...
| Roles y permisos (7.24) | ⛔ | **Handlers de contabilidad sin control de rol** |
...
- **F2 — Gobierno contable:** control de rol en handlers `accounting:*`/`financial:*` (G2);
  cierre de período + bloqueo de asientos en período cerrado (G4). *Riesgo bajo, aditivo.*
- **F3 — Compras → contabilidad (G1, G7):** `generatePurchaseEntry` (Déb Inventario/Gasto/
  Activo + ITBIS adelantado · Créd Caja/Banco o CxP), enganche en `purchases:create/receive`;
  gasto "por pagar" → CxP contable; pago proveedor → Déb CxP/Créd Caja. Backfill en sync.
